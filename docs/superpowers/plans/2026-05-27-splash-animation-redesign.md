# Splash Animation Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the current platform-split splash screen (video on native, simple fade on web) with a single unified "Neon Ignite" animation — a glowing spark traces the mountain logo, it ignites, and "ASCENT" stagger-compresses in.

**Architecture:** Full rewrite of `components/AnimatedSplashScreen.tsx`. One component, no Platform split, no video file. Uses React Native `Animated` + `react-native-svg`. SVG prop animations run with `useNativeDriver: false`; all other animations use `useNativeDriver: true` — they must never be mixed in the same `Animated.parallel()`.
...
| File | Change |
|------|--------|
| `components/AnimatedSplashScreen.tsx` | Full rewrite |

`app/_layout.tsx` — no changes. The `onAnimationComplete` prop interface is unchanged.

---

## Task 1: Rewrite AnimatedSplashScreen

**Files:**
- Modify: `components/AnimatedSplashScreen.tsx`

- [ ] **Step 1: Read the current file**

Open `components/AnimatedSplashScreen.tsx` and note the current exports. The only thing we preserve is:
```ts
interface Props {
...
Expected: 0 errors. If you see an error about `strokeDashoffset` type, confirm the `as any` cast is present on both `AnimatedPath` elements. TypeScript doesn't know that `Animated.Value` is valid for SVG numeric props — this cast is intentional and safe.

- [ ] **Step 4: Run on web and check the animation**
...
1. Dark screen (~0.5s)
2. Bright spark starts at the top of the mountain and travels the full outline, leaving a dimmer cyan trail permanently
3. When the spark finishes, the filled green logo snaps in with a ring that expands and fades
4. "ASCENT" letters appear one by one sliding up from slightly below
5. "LEVEL UP YOUR FITNESS" fades in beneath
6. Everything fades out → main app loads

If the spark travels but leaves no trail (looks like a snake), the `strokeDasharray` on the trail path is wrong — verify it reads `"950"` (one value = one dash of full path length).

If the logo doesn't snap in, check that `fillOpacity` starts at `0` and the `Animated.delay(IGNITE_AT)` in Group B is correct (should be `2500`).

- [ ] **Step 5: Commit**

```bash
git add components/AnimatedSplashScreen.tsx
git commit -m "feat: redesign splash with Neon Ignite animation

Replace platform-split video/fade splash with unified code-based animation.
Spark traces mountain logo outline, logo ignites, ASCENT letters stagger in.
Removes expo-av Video dependency (was deprecated)."
```
